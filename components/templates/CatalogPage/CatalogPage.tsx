...
const CatalogPage = ({ query }: { query: IQueryParams }) => {
    ...
    const q = router.query as Partial<IQueryParams>
    const isValidOffset = q.offset && !isNaN(+q.offset) && +q.offset > 0
    ...
    const filtersQuery = `${
        q.boiler ? `&boiler=${q.boiler}` : ''
    }${
        q.parts ? `&parts=${q.parts}` : ''
    }${
        q.priceFrom && q.priceTo ? `&priceFrom=${q.priceFrom}&priceTo=${q.priceTo}` : ''
    }`
    const { toggleOpen, open, closePopup } = usePopup()

    useEffect(() => {
        if (!router.isReady) return
        const nextPage = isValidOffset ? +q.offset! - 1 : 0
        setCurrentPage(nextPage)
        loadBoilerParts()
    }, [router.isReady, router.query.offset, router.query.boiler, router.query.parts, router.query.priceFrom, router.query.priceTo])  

    const loadBoilerParts = async () => {
        try {
            setSpinner(true)
            const firstPageData = await getBoilerPartsFx(`/boiler-parts?limit=20&offset=0${filtersQuery}`)
            ...
            if (!isValidOffset) {
                ...
                resetPagination(firstPageData)
                return
            }

            if (isValidOffset) {
                if (+q.offset! > Math.ceil(firstPageData.count / 20)) {
                    router.replace({
                        query: {
                            ...router.query,
                            offset: 1,
                    ...
                    setCurrentPage(0)
                    setBoilerParts(firstPageData)
                    return
                }

                const offset = +q.offset! - 1
                const result = await getBoilerPartsFx(`/boiler-parts?limit=20&offset=${offset}${filtersQuery}`)
    ...
    const handlePageChange = async ({selected}: {selected: number}) => {
        try {
            ...
            if (isValidOffset && q.offset && +q.offset > Math.ceil(firstPageData.count / 20)) {
                resetPagination(firstPageData)
                return
            }
